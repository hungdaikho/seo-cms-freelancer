...
export const getPlanTableFilters = (plans: AdminSubscriptionPlan[]) => {
    const supports = [...new Set(plans.map(plan => plan.features.support))];
    const statuses = [
        { text: 'Active', value: true },
        { text: 'Inactive', value: false },
    ];

    const supportFilters = supports.map(support => ({
        text: support.charAt(0).toUpperCase() + support.slice(1),
        value: support,
    }));

    return { supportFilters, statuses };
};

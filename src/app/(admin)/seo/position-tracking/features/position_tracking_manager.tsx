...
const PositionTrackingManager: React.FC = () => {
  const selectedProject = useSelector((state: RootState) => state.project.currentProject);
  const [trackingData, setTrackingData] = useState<TrackingProject | null>(null);
  ...
  const filteredKeywords = trackingData?.keywords.filter(keyword => {
    const matchesSearch = keyword.keyword.toLowerCase().includes(searchKeyword.toLowerCase());
    const matchesTags = selectedTags.length === 0 || selectedTags.some(tag => keyword.tags.includes(tag));
  ...
  const keywordColumns = [
    {
      ...
      dataIndex: 'keyword',
      key: 'keyword',
      sorter: (a: KeywordRanking, b: KeywordRanking) => a.keyword.localeCompare(b.keyword),
    ...
    },
    {
      ...
      key: 'position',
      sorter: (a: KeywordRanking, b: KeywordRanking) => a.currentPosition - b.currentPosition,
      render: (position: number, record: KeywordRanking) => {
        const change = getPositionChange(record.currentPosition, record.previousPosition);
        return (
          <div className={styles.positionCell}>
            <span className={styles.currentPosition}>{position}</span>
            {change.value !== 0 && (
              <span className={`${styles.positionChange} ${
    ...
    },
    {
      ...
      key: 'volume',
      sorter: (a: KeywordRanking, b: KeywordRanking) => a.searchVolume - b.searchVolume,
    ...
    },
    {
      ...
      key: 'difficulty',
      sorter: (a: KeywordRanking, b: KeywordRanking) => a.difficulty - b.difficulty,
      render: (difficulty: number) => (
        <Progress 
          percent={difficulty} 
          size="small" 
          strokeColor={difficulty > 70 ? '#ff4d4f' : difficulty > 40 ? '#faad14' : '#52c41a'}
          showInfo={true}
  ...
  const tabItems = [
    {
      ...
      children: (
        <div className={styles.overviewTab}>
          ...
          <Card title="Position Distribution" className={styles.distributionCard}>
            <Row gutter={16}>
              <Col span={6}>
                <Statistic 
                  title="Top 3" 
                  value={filteredKeywords.filter(k => k.currentPosition <= 3).length}
                  valueStyle={{ color: '#52c41a' }}
                />
              </Col>
              <Col span={6}>
              ...
              <Col span={6}>
                <Statistic 
                  title="Top 20" 
                  value={filteredKeywords.filter(k => k.currentPosition <= 20).length}
      ...
      children: (
        <div className={styles.competitorsTab}>
          <Card title="Competitor Analysis">
            ...
            <p>This feature shows how your keywords perform against competitor rankings.</p>
          </Card>
        </div>
      ),
    },
      ...
      children: (
        <div className={styles.reportsTab}>
          <Card title="Ranking Reports">
            ...
            <p>Generate PDF reports, schedule email reports, and track historical performance.</p>
          </Card>
        </div>
      ),
    },

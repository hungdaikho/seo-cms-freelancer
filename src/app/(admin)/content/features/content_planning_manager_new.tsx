...
const ContentPlanningManager: React.FC<ContentPlanningManagerProps> = () => {
  ...
  const handleGenerateIdeas = async (values: any) => {
    console.log("🎨 Generating content ideas with values:", values);
    try {
      const data: ContentIdeaRequest = values;
      const result = await dispatch(generateContentIdeas(data)).unwrap();
      console.log("✅ Content ideas generated successfully:", result);
    ...
    <div style={{ padding: "24px" }}>
            ...
            {JSON.stringify(
              {
                ...
                contentIdeasStructure: {
                  hasContentIdeas: !!contentIdeas,
                  hasResult: !!contentIdeas?.result,
                  ideasCount: contentIdeas?.result?.ideas?.length || 0,
                  pillarsCount:
                    contentIdeas?.result?.contentPillars?.length || 0,
                  seasonalCount:
                    contentIdeas?.result?.seasonalTopics?.length || 0,
                  requestId: contentIdeas?.requestId,
                },
              },
              null,
              2
      ...
      <Card>
        <Tabs activeKey={activeTab} onChange={setActiveTab}>
          ...
          >
            <Row gutter={[16, 16]}>
              {contentIdeas && contentIdeas.result && (
                <>
                  {/* Content Ideas */}
                  {contentIdeas.result.ideas &&
                    contentIdeas.result.ideas.length > 0 && (
                      <Col span={12}>
                        <Card
                          title={`Content Ideas (${contentIdeas.result.ideas.length})`}
                          extra={
                            <Button
                              size="small"
                        ...
                        >
                          <List
                            dataSource={contentIdeas.result.ideas}
                            renderItem={(idea: any) => (
                              <List.Item
                                actions={[
                                  <Tag
                  ...
                  {/* Content Pillars */}
                  {contentIdeas.result.contentPillars &&
                    contentIdeas.result.contentPillars.length > 0 && (
                      <Col span={12}>
                        <Card
                          title={`Content Pillars (${contentIdeas.result.contentPillars.length})`}
                          extra={
                            <Button
                              size="small"
                              onClick={() => dispatch(clearContentIdeas())}
                        ...
                        >
                          <List
                            dataSource={contentIdeas.result.contentPillars}
                            renderItem={(pillar: any, index) => (
                              <List.Item>
                                <List.Item.Meta
                                  avatar={
                                    <div
                                  ...
                                  title={pillar || ""}
                                  description={`Strategic content pillar ${
                                    index + 1
                                  }`}
                                />
                              </List.Item>
                  ...
                  {/* Seasonal Topics */}
                  {contentIdeas.result.seasonalTopics &&
                    contentIdeas.result.seasonalTopics.length > 0 && (
                      <Col span={24}>
                        <Card
                          title={`Seasonal Topics (${contentIdeas.result.seasonalTopics.length})`}
                        >
                          <List
                            dataSource={contentIdeas.result.seasonalTopics}
                            renderItem={(seasonal: any) => (
                              <List.Item
                                actions={[
                                  <Tag key="season" color="orange">
                                    {seasonal.season}
                                  </Tag>,
                                ]}
                              >
                                <List.Item.Meta
                                  ...
                                  description={seasonal.opportunity}
                                />
                              </List.Item>
                            )}
                          />
              ...
              {/* Show message when content ideas are available but empty */}
              {contentIdeas &&
                contentIdeas.result &&
                (!contentIdeas.result.ideas ||
                  contentIdeas.result.ideas.length === 0) &&
                ...
                (!contentIdeas.result.seasonalTopics ||
                  ...
                  <Col span={24}>
                    <Card>
                      <Empty
                        description="No content ideas found for your criteria"
                        image={Empty.PRESENTED_IMAGE_SIMPLE}
                      >
                        <Space direction="vertical">
                          <Text type="secondary">
                            Try adjusting your topic, audience, or format to get
                            better results.
                          </Text>
              ...
              {/* Show empty state when no research data is available at all */}
              {(!contentIdeas || !contentIdeas.result) &&
                !topicIdeas &&
                !topicQuestions && (
                  <Col span={24}>

...
export class AuthService extends BaseService {
  ...
  initiateGoogleAuth(): void {
    const googleAuthUrl = `${getBaseUrl()}/api/v1/auth/google`;
  ...
  updateAvatarFromUrl(avatarUrl: string): Promise<UploadAvatarResponse> {
    return this.post<UploadAvatarResponse>("/users/avatar-from-url", {
      avatarUrl,
    });
  }
  ...
  getUserActivity(
    ...
    const queryString = params
      ...
      : "";
    return this.get<ApiResponse<UserActivity[]>>(
  ...
  enableTwoFactor(): Promise<{
    ...
    return this.post<{ qrCode: string; secret: string; backupCodes: string[] }>(
  ...
  verifyTwoFactor(token: string): Promise<{ backupCodes: string[] }> {
    return this.post<{ backupCodes: string[] }>("/users/security/2fa/verify", {
      token,
    });
  }
  ...
  generateBackupCodes(): Promise<{ backupCodes: string[] }> {
    return this.post<{ backupCodes: string[] }>(
      "/users/security/backup-codes/generate",
      {}
    );
  }
  ...
  getLoginHistory(params?: PaginationParams): Promise<ApiResponse<any[]>> {
    ...
    return this.get<ApiResponse<any[]>>(
      `/users/security/login-history${queryString}`
    );
  }
  ...
  requestAccountDeletion(
    reason?: string
  ): Promise<{ message: string; deletionDate: string }> {
    return this.post<{ message: string; deletionDate: string }>(
  ...
  validateSession(): Promise<{ valid: boolean; expiresAt?: string }> {
    return this.get<{ valid: boolean; expiresAt?: string }>(
  ...
  checkUsernameAvailability(username: string): Promise<{ available: boolean }> {
    return this.post<{ available: boolean }>("/auth/check-username", {
      username,
    });
  }
}
